    ...
    div.innerHTML = `
          <div class="single-product">
            <div>
              ...
              <div class="rating">
                <i class="fas fa-star rate">${product.rating.rate}</i>
                <p class="count">(${product.rating.count})</p>
              </div>
              <h2>Price: $ ${product.price}</h2>
              <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
              <button onclick="detailsCart()" id="details-btn" class="btn btn-danger">Details</button>
          </div>
      `;
...
const updateTotal = () => {
  ...
  document.getElementById('total').innerText = parseInt(grandTotal).toFixed(2);
